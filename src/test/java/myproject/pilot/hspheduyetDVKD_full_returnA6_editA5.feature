@regression @hspheduyetDVKD_full_returnA6_editA5
Feature: Hồ sơ phê duyệt tại Đơn vị kinh doanh - full luồng

  @hspheduyetDVKD_full_returnA6_editA5-01
  Scenario Outline: register Hồ sơ thẻ phê duyệt tại Đơn vị kinh doanh with cif number
    ...
    And I fill all data in Thông tin thẻ tín dụng
      | CN/PGD HDBank | <client code> | Email | <fee policy> | Địa chỉ thường trú | CN PHU NHUAN | <card type> | Không trích nợ tự động | QC TEST | 011252487 | 0983234167 | 12 |TN04 - TN từ lương |
    ...
    Examples:
      ...
      | 01357972 | 001-Local Credit Gold | RGL         | Miễn 50% phí thường niên | 50000000 | Hồ sơ nhanh |

  @hspheduyetDVKD_full_returnA6_editA5-02
  ...
  @hspheduyetDVKD_full_returnA6_editA5-03
  ...
  @hspheduyetDVKD_full_returnA6_editA5-04
  ...
  @hspheduyetDVKD_full_returnA6_editA5-05
  ...
  @hspheduyetDVKD_full_returnA6_editA5-06
  Scenario: approve with user a4nv
    Given I access link with "a4nv" and "Abc123!@#"
    When I search with the above request code from "QLPheDuyetYeuCau"
    And I send request with the above file code from A4 user
  ...
  @hspheduyetDVKD_full_returnA6_editA5-07
  ...
  @hspheduyetDVKD_full_returnA6_editA5-08
...
#      | Tiếp nhận hồ sơ    |
  ...
  @hspheduyetDVKD_full_returnA6_editA5-09
  Scenario: approve with user a5nv
    Given I access link with "a5nv" and "Abc123!@#"
    When I search with the above request code from "QLPheDuyetYeuCau"
    ...
    And I edit information in Thông tin thẻ tín dụng
      | Địa chỉ công ty | SALE_ONLINE | Bưu điện | Miễn 50% phí thường niên | Địa chỉ thường trú | Chi nhánh Lãnh Binh Thăng | 001-Visa Credit Vietjet SOVICO VIP | RGL | Không trích nợ tự động | 24 |TN04 - TN từ chủ DN |
    And I edit information at Thông tin thẩm định as below
      | Nhà thường trú | 700000000 | Đồng ý | A01 - Quy định |
    And I edit information in Thông tin phê duyệt as below
      | Phê duyệt qua HDTĐ | A5 |600000000 | Đồng ý với điều kiện |
    Then I save all information
    And I send additional request with the above file code from A5 user
    Then I input "huyenlm2@hdbank.com.vn" to approve

  @hspheduyetDVKD_full_returnA6_editA5-10
