...
Feature: Hồ sơ phê duyệt tại Đơn vị kinh doanh - full luồng
  ...
  Scenario Outline: register Hồ sơ thẻ phê duyệt tại Đơn vị kinh doanh with cif number
    ...
    And I fill all data in Thông tin thẻ tín dụng
      | CN/PGD HDBank | <client code> | Email | <fee policy> | Địa chỉ thường trú | CN LANH BINH THANG | <card type> | Không trích nợ tự động | QC TEST | 011252487 | 0983234167 | 12 |TN04 - TN từ lương |
    ...
    Examples:
      ...
      | 01357983 | 001-Local Credit Gold | RGL         | Miễn 50% phí thường niên | 50000000 | Hồ sơ nhanh |
...
#      | Thẩm định hồ sơ    |
  ...
  Scenario: approve with user a3nv
    ...
    And I edit information in Thông tin phê duyệt as below
      | Phê duyệt qua HDTĐ | A1 | 600000000 | Đồng ý với điều kiện |
    Then I save all information
    And I send additional request with the above file code from A3 user
    Then I input "huyenlm2@hdbank.com.vn" to approve from A3
